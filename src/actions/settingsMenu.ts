...
@action({ UUID: "xyz.hajsori.simplemacro.streamdeck.settings-menu" })
export class SettingsMenuAction extends SingletonAction<KeySettings> {
    ...
    override async onKeyDown(event: KeyDownEvent): Promise<void> {
        ...
        for (const ws of sockets) {
            try {
                ws.send(messages.setScreen.settings);
            } catch {
                await event.action.showAlert();
            }
        }
