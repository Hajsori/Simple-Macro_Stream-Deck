...
@action({ UUID: "xyz.hajsori.simplemacro.streamdeck.leave-group" })
export class LeaveGroupAction extends SingletonAction<KeySettings> {
    ...
    override async onKeyDown(event: KeyDownEvent<KeySettings>): Promise<void> {
        ...
        for (const ws of sockets) {
            ws.send(messages.deactivate.leaveGroup);
        }
    }
}
