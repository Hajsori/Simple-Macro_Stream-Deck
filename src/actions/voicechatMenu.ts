...
@action({ UUID: "xyz.hajsori.simplemacro.streamdeck.voicechat-menu" })
export class VoicechatMenuAction extends SingletonAction<KeySettings> {
    ...
    override async onKeyDown(event: KeyDownEvent): Promise<void> {
        ...
        for (const ws of sockets) {
            try {
                ws.send(messages.setScreen.voiceChat);
            } catch {
                await event.action.showAlert();
            }
        }
